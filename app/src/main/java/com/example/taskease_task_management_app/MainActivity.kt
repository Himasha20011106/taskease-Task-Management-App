...
import com.example.taskease_task_management_app.adapters.TaskRecyclerViewListAdapter
import com.example.taskease_task_management_app.databinding.ActivityMainBinding
import com.example.taskease_task_management_app.models.Task
import com.example.taskease_task_management_app.utils.Status
import com.example.taskease_task_management_app.utils.StatusResult
import com.example.taskease_task_management_app.utils.StatusResult.Added
import com.example.taskease_task_management_app.utils.StatusResult.Deleted
import com.example.taskease_task_management_app.utils.StatusResult.Updated
import com.example.taskease_task_management_app.utils.clearEditText
import com.example.taskease_task_management_app.utils.hideKeyBoard
import com.example.taskease_task_management_app.utils.longToastShow
import com.example.taskease_task_management_app.utils.setupDialog
import com.example.taskease_task_management_app.utils.validateEditText
import com.example.taskease_task_management_app.viewmodel.TaskView
...
class MainActivity : AppCompatActivity() {
    ...
    private val addTaskDialog: Dialog by lazy {
        Dialog(this, R.style.DialogCustomTheme).apply {
            setupDialog(R.layout.add_task_box)
        }
    }

    private val updateTaskDialog: Dialog by lazy {
        Dialog(this, R.style.DialogCustomTheme).apply {
            setupDialog(R.layout.update_task_box)
        }
    }
    ...
    private val taskViewModel: TaskView by lazy {
        ViewModelProvider(this)[TaskView::class.java]
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val saveTaskBtn = addTaskDialog.findViewById<Button>(R.id.saveTaskButton)
        ...
        isListMutableLiveData.observe(this){
            if (it){
                ...
                mainBinding.listOrGridImg.setImageResource(R.drawable.baseline_view_module_24)
            }else{
                mainBinding.taskRV.layoutManager = StaggeredGridLayoutManager(
                    2,LinearLayoutManager.VERTICAL
                )
                mainBinding.listOrGridImg.setImageResource(R.drawable.baseline_view_list_24)
            }
        }
        ...
        val taskRVVBListAdapter = TaskRecyclerViewListAdapter(isListMutableLiveData ) { type, position, task ->
            if (type == "delete") {
                taskViewModel
                    // Deleted Task
//                .deleteTask(task)
...
//                mainBinding.taskRV.smoothScrollToPosition(positionStart)
        ...
        mainBinding.edSearch.addTextChangedListener(object : TextWatcher{
            ...
            override fun afterTextChanged(query: Editable) {
                if (query.toString().isNotEmpty()){
                    taskViewModel.searchTasks(query.toString())
                }else{
                    callSortByLiveData()
                }
            }
    ...
    private fun callGetTaskList(taskRecyclerViewAdapter: TaskRecyclerViewListAdapter) {
